...
- 全生命周期可玩：建角→决策罗盘/自由输入→战斗（命运骰子演出）→秘境游历→坊市议价→百艺经营→道缘往来→渡劫突破→天命六阶→终章封号→轮回转世
- 天机简报/未竟仙途/仙途史册（只读防 SL）完整
- /admin 运营看板（LLM 成本/降级率/存档漏斗）
- 待办：因缘际会 6-7 月事件接入、道统传承、PWA/推送、数值表后台 CRUD

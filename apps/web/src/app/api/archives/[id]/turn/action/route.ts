...
const ActionSchema = z.object({ turnNo: z.number().int().min(0), optionIdx: z.number().int().min(1).max(15) });

export async function POST(req: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    const deviceId = await getOrCreateDeviceId();
    const { id } = await params;
    const { turnNo, optionIdx } = ActionSchema.parse(await req.json());
    return NextResponse.json(await submitAction(id, deviceId, turnNo, optionIdx));
  } catch (e) {
